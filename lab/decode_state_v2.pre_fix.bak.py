    ...
    while r.p < len(data):
        ...
        try:
            ...
            if op == 1:   # Pop
                ...
                top[f] = val
                # If we're inside an entity context, mirror to entity_store
                ...
                if isinstance(cid, int) and cid in doc.models:
                    stack.append(cid)
                    # Detect World push from Root
                    if cid == world_id:
                        ctx_stack.append(("world",))
                    else:
                        ctx_stack.append(None)
                else:
                    # Missing ref: create a placeholder model of type -1
                    # so the stack doesn't corrupt
                ...
                top[f] = cid
                stack.append(cid)
                # Detect World creation at Root.field0
                nonlocal_world = None
            ...
            elif op == 6:   # AssignKey (scalar map)
                f = r.u8()
                k = r.i32()
                vt, ism, scal = finfo_for(top_id, f)
                val = read_value(r, vt, ism, scal)
            ...
            elif op == 7:   # PushKey (existing child)
                f = r.u8()
                k = r.i32()
                cid = top.get(f, {}).get(k)
                if isinstance(cid, int) and cid in doc.models:
                    stack.append(cid)
                    # Entering World.entities[k]?
                    if ctx == ("world",) and f == 1:
                        ctx_stack.append(("entity", k))
                        # Ensure entity_store entry exists
                        if k not in entity_store:
                            entity_store[k] = {"__type__": doc.models[cid]["__type__"]}
                    else:
                        ctx_stack.append(None)
                else:
                    # Entity exists in store but was created in snapshot -> add placeholder
                    new_id = doc.register(
            ...
            elif op == 8:   # PushCreateAndAssignKey (entity CREATE for World.entities)
                f = r.u8()
                mt = r.u8()
                k = r.i32()
                cid = doc.register(mt)
            ...
            elif op == 9:   # ResetKey (entity DEATH for World.entities)
                f = r.u8()
                k = r.i32()
                m = top.get(f)
            ...
            r.p = op_pos + 1
            resyncs += 1
            # If we over-popped, reset to a safe depth
